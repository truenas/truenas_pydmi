...
class TestH30Supermicro:
    ...
    def test_chassis(self):
        ...
        assert c.type == 17
        assert self.dmi.chassis_type_name(c.type) == "Main Server Chassis"
    ...
    def test_cache_lookup_by_handle(self):
        ...
        l1 = self.dmi.cache(p.l1_cache_handle)
        l2 = self.dmi.cache(p.l2_cache_handle)
        l3 = self.dmi.cache(p.l3_cache_handle)
    ...
    def test_cache_lookup_unknown_handle(self):
        assert self.dmi.cache(0xDEAD) is None

    def test_memory_array(self):
        arrays = self.dmi.memory_arrays()
        assert len(arrays) == 1
        a = arrays[0]
        assert self.dmi.memory_error_correction_name(a.error_correction) == "Single-bit ECC"
        assert self.dmi.is_ecc(a.error_correction) is True
    ...
    def test_memory_devices(self):
        ...
        for d in devs:
            ...
            assert self.dmi.memory_type_name(d.type) == "DDR4"
    ...
    def test_ipmi(self):
        ...
        assert len(devs) == 1
        ipmi = devs[0]
        assert self.dmi.ipmi_interface_type_name(ipmi.interface_type) == "KCS (Keyboard Control Style)"
...
class TestF60Viking:
    ...
    def test_amd_epyc_zen_family(self):
        ...
        assert self.dmi.processor_family_name(p.family) == "Zen"
...
class TestM50SupermicroX11:
    ...
    def test_dual_socket_processors(self):
        ...
        for p in procs:
            ...
            assert self.dmi.processor_family_name(p.family) == "Xeon"
            assert p.populated is True
            assert p.voltage_volts == 1.6
